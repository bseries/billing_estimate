...
namespace billing_estimate\documents;
...
/**
 * An estimate document to be printed on a blank paper with no header/footer.
 */
class Estimate extends \billing_core\documents\BaseFinancial {

	protected $_layout = 'estimate';
	...
	protected function _compileNumbers() {
		...
		$this->_drawText($t('{:number} — Estimate No.', [
			'scope' => 'base_document',
			'locale' => $this->_recipient->locale,
	...
	protected function _compileTableFooter() {
		...
		$this->_currentHeight = $this->_skipLines(2);
		$text = $t("This estimate has been automatically generated and is valid even without a signature.", [
			'scope' => 'base_document',
			'locale' => $this->_recipient->locale
		]);
		$this->_drawText($text);
